...
export default function Appointments() {
  ...
  useEffect(() => {
    api.get('/appointments')
       ...
       .catch(() => alert('Failed loading appointments'));

    api.get('/patients')
       .then(r => setPatients(r.data || []))
       .catch(() => {});

    // fetch all physicians (including their specialization)
    api.get('/users/physicians')
       .then(r => setPhysicians(r.data || []))
       .catch(() => {});
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      setAppointments(a => a.map(x =>
        x._id === id ? { ...x, status: data.status } : x
      ));
    } catch {
  ...
  const handleOpenVisit = async appt => {
    if (!appt.started) {
      try {
        ...
        setAppointments(a =>
          a.map(x => x._id === appt._id ? { ...x, started } : x)
        );
      } catch {}
    }
    navigate('/sigmapanel', {
      ...
      <div className="w-full flex flex-col lg:flex-row gap-3">
        ...
        <form onSubmit={addAppointment}
          ...
          <div className="space-x-4">
            <button type="button"
                    className="bg-accent-200 text-on-accent hover:bg-accent-200 focus:outline-none focus:ring-2 focus:ring-accent-300 focus:ring-offset-2 font-medium px-5 py-2 rounded-full">
              Appointment
            </button>
            <button type="button"
                    onClick={() => navigate('/patient')}
                    className="border-2 border-accent-100 text-on-accent bg-transparent hover:bg-accent-100/10 focus:outline-none focus:ring-2 focus:ring-accent-300 focus:ring-offset-2 font-medium px-5 py-2 rounded-full">
              Add Patient
            </button>
          </div>
          ...
          <div className="relative">
            <input
              ...
              required
              className="block w-full text-gray-dark placeholder-gray-base bg-white border border-gray-base rounded-lg px-4 py-2 hover:bg-accent-100/10 focus:outline-none focus:ring-2 focus:ring-accent-200 focus:border-accent-100"
            />
            {fPatients.length > 0 && (
          ...
          <select
            ...
            onChange={handleChange}
            required
            className="block w-full text-gray-dark bg-white border border-gray-base rounded-lg px-4 py-2 hover:bg-accent-100/10 focus:outline-none focus:ring-2 focus:ring-accent-200 focus:border-accent-100"
          >
          ...
          <div className="flex space-x-4">
            <button type="submit"
                    className="flex-1 bg-accent-100 text-on-accent hover:bg-accent-200 focus:outline-none focus:ring-2 focus:ring-accent-300 focus:ring-offset-2 font-semibold px-4 py-2 rounded-full">
              Save
            </button>
            <button type="button" onClick={clearForm}
                    className="flex-1 bg-transparent border-2 border-red-500 text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-200 focus:ring-offset-2 font-medium px-4 py-2 rounded-full">
              Clear
            </button>
          </div>
        </form>
        ...
        <div className="w-full lg:w-3/5 bg-white p-6 rounded-lg shadow flex flex-col">
          <div className="flex justify-between items-center mb-5">
            <input
              ...
              className="block text-gray-dark bg-white border border-gray-base rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-accent-200 focus:border-accent-100 cursor-pointer hover:bg-accent-100/10 rounded-full"
            />
            <div className="flex space-x-4">
              <select
                value={locFil}
                onChange={e => setLocFil(e.target.value)}
                className="block text-gray-dark bg-white border border-gray-base cursor-pointer hover:bg-accent-100/10 rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-accent-200 focus:border-accent-100"
              >
              ...
              <select
                value={statFil}
                onChange={e => setStatFil(e.target.value)}
                className="block text-gray-dark bg-white border border-gray-base cursor-pointer hover:bg-accent-100/10 rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-accent-200 focus:border-accent-100"
              >
                  ...
                  <li key={appt._id} className="p-6 border rounded-lg space-y-4 hover:bg-accent-100/10">
                    <div className="flex items-center justify-between">
                      ...
                      <select
                        ...
                        className="bg-transparent border-2 border-accent-100 text-on-accent hover:border-accent-200 hover:bg-accent-200/10 focus:outline-none focus:ring-2 focus:ring-accent-300 focus:ring-offset-2 px-6 py-2 rounded-full"
                      >
                        {statusOptions.filter(o => o.value !== 'all').map(o => (
                          <option key={o.value} value={o.value}>{o.label}</option>
                        ))}
